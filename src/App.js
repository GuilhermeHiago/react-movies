...
function App() {
  ...
  function makeGenresString(){
    ...
    // for a AND filter should use ',' to conect two filters, for OR use "|"
    currentFilters.map(genre => str += `${filters[genre]},`)
  ...
  async function getPopularMovies(){
    ...
    let url = `movie/popular?api_key=${api_key}&${makeGenresString(currentFilters)}&language=pt-BR&page=${currentPage}`
  ...
  function handleFilterClick(filterName, pressed){
    if(pressed){
      setCurrentFilters(currentFilters.concat([filterName]))
    }
    else{
      setCurrentFilters(currentFilters.filter(value => value !== filterName))
    }
  }
    ...
    <div>
      {
        isMovieView === true ? (
          <div className="App">
            ...
            <header className="App-header text-wrap mw-100">
          ...
          <div className="App">
            ...
            <header className="App-header text-wrap mw-100">
              ...
              <p>Filtre por:</p>
              <FilterList items={Object.keys(filters)} currentFilters={currentFilters} onFilterClick={handleFilterClick}></FilterList>
            </header>
